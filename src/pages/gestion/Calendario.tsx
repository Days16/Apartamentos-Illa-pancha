              ...
              {filteredApts.map((apt) => (
                <tr key={apt.slug} className="hover:bg-gray-50/50 group">
                  ...
                  {timelineDates.map((date, idx) => {
                    ...
                    const isCheckin = res && new Date(res.checkin + 'T00:00:00').toDateString() === date.toDateString();
                    const isToday = new Date().toDateString() === date.toDateString();
                    const isSelected = selectedDate && new Date(selectedDate + 'T00:00:00').toDateString() === date.toDateString();
        ...
        <div className="mt-8 bg-white border border-gray-200 rounded-xl shadow-sm p-6 animate-in slide-in-from-bottom-4 duration-300">
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-xl font-bold text-navy">
              Reservas para el {new Date(selectedDate + 'T00:00:00').toLocaleDateString('es-ES', { day: 'numeric', month: 'long', year: 'numeric' })}
            </h3>
            <button
                  ...
                  <div key={res.id} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                    ...
                    <div className="space-y-2 mb-4">
                      <div className="flex items-center gap-2 text-sm text-gray-600">
                        ...
                        <span>{new Date(res.checkin + 'T00:00:00').toLocaleDateString()} - {new Date(res.checkout + 'T00:00:00').toLocaleDateString()}</span>
                      </div>
                      <div className="flex items-center gap-2 text-sm text-gray-600">
                        <span className="w-5 text-gray-400">💰</span>
