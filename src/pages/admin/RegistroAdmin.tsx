...
export default function RegistroAdmin() {
  ...
  useEffect(() => {
    Promise.all([fetchSettings(), fetchAllApartments()]).then(([settings, apts]) => {
      ...
      FIELDS.forEach(f => { v[f.key] = (settings[f.key] as string) || ''; });
      setValues(v);
      setApartments(apts || []);
  ...
  const handleSave = async () => {
    setSaving(true);
    await Promise.all(
      FIELDS.map(f => updateSetting(f.key, values[f.key] || '', 'string'))
    );
    setSaving(false);
    setSaved(true);
  ...
  return (
    <div className="p-4 md:p-6 space-y-6">
          ...
          <table className="w-full border-collapse text-[10px]">
            <tbody>
              {[
                ...
                <tr key={i}>
                  <td className="border border-gray-400 dark:border-gray-600 px-2 py-2 w-1/2 text-gray-500 dark:text-gray-400">{a}</td>
                  <td className="border border-gray-400 dark:border-gray-600 px-2 py-2 w-1/2 text-gray-500 dark:text-gray-400">{b}</td>
                </tr>
              ))}
              <tr>
                <td colSpan={2} className="border border-gray-400 dark:border-gray-600 px-2 py-3 text-gray-500 dark:text-gray-400">
                  Firma / Signature
                </td>
              </tr>
            </tbody>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl p-5 shadow-sm border border-gray-100 dark:border-gray-700 space-y-5">
        <h2 className="text-sm font-semibold text-navy dark:text-white">Configuración del documento</h2>
      ...
      <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl p-4">
        <p className="text-xs text-amber-800 dark:text-amber-300 leading-relaxed">
          <strong>Obligación legal:</strong> En España, los establecimientos de alojamiento turístico deben
          registrar los datos de los viajeros y comunicarlos a la Guardia Civil o Policía Nacional
          (Orden INT/1922/2003). Este documento es la hoja de registro oficial que firman los huéspedes al hacer check-in.
          Guarda los registros al menos 3 años.
        </p>
      </div>
    </div>
  );
