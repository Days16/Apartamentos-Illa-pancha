...
const RULE_TYPES = [
  { id: 'lead_time', label: 'Antelación', unit: 'días antes', desc: 'Días entre la reserva y el checkin' },
  { id: 'occupancy', label: 'Ocupación', unit: '% ocupación', desc: '% de apartamentos ocupados ese mes' },
  { id: 'time_of_day', label: 'Hora del día', unit: 'hora (0–23)', desc: 'Hora local en que se realiza la reserva' },
];
...
export default function Precios() {
  ...
  async function handleSave(e) {
    e.preventDefault();
    if (!form.label.trim()) { setFormError('La descripción es obligatoria.'); return; }
    if (form.modifier === '' || form.modifier === null) { setFormError('El modificador (%) es obligatorio.'); return; }
    if (form.condition_min === '') { setFormError('El valor mínimo es obligatorio.'); return; }
    ...
    try {
      if (editingId) {
        const { error } = await supabase.from('dynamic_pricing_rules').update(payload).eq('id', editingId);
      ...
      <div className="border-b border-gray-200 px-8 pt-8 pb-6 flex justify-between items-start bg-slate-50">
        <div>
          ...
          <div className="text-sm text-gray-400 mt-1">
            Reglas de modificación de precio por antelación, ocupación y hora del día.
            El precio final = precio base × (1 + suma de modificadores activos / 100).
          </div>
        </div>
        <button
          onClick={startNew}
      ...
      <div className="p-8">
        ...
        {showForm && (
          <div className="bg-white border border-gray-200 rounded-lg shadow-sm p-6 mb-8">
            ...
            {formError && (
              <div className="mb-4 px-4 py-3 bg-red-50 border border-red-200 text-red-700 text-sm rounded-lg">{formError}</div>
            )}

            <form onSubmit={handleSave} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-slate-600 mb-1.5">Tipo de regla *</label>
                  <select
                    value={form.type}
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder={
                      form.type === 'lead_time' ? 'Ej: Reserva de última hora'
                        : form.type === 'occupancy' ? 'Ej: Alta demanda >80%'
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-600 mb-1.5">
                    Valor máximo ({RULE_TYPES.find(t => t.id === form.type)?.unit}, vacío = sin límite)
                  </label>
                  <input
                    type="number"
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-600 mb-1.5">
                    Modificador (%) * <span className="text-gray-400 font-normal">— positivo = sube, negativo = baja</span>
                  </label>
                  <input
                    type="number"
                    step="0.1"
              ...
              <div className="flex items-center gap-3 pt-2">
                ...
                <span className="text-sm text-slate-600">{form.active ? 'Regla activa' : 'Regla desactivada'}</span>
              </div>
            ...
            {rulesByType.map(group => (
              <div key={group.id}>
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <span className="ml-auto text-xs text-gray-400">{group.rules.filter(r => r.active).length} activas</span>
                </div>
        ...
        <div className="mt-8 bg-slate-50 border border-slate-200 rounded-lg p-5 text-sm">
          ...
          <ul className="space-y-1 text-slate-600 text-xs list-disc list-inside">
            <li>Se evalúan todas las reglas activas que apliquen a la reserva en el momento de crearla.</li>
            <li>Los modificadores se suman: si aplican +20% y -10%, el precio sube un 10%.</li>
            <li>La lógica de cálculo se implementa en la Edge Function <code className="bg-white px-1 rounded font-mono border border-slate-200">dynamic-pricing</code>.</li>
            <li>En el widget de reserva se muestra el precio base tachado y el precio dinámico final.</li>
          </ul>
        </div>
      </div>
    </div>
