...
export default function Cancelacion() {
  ...
  async function loadData() {
    ...
    const [settings, { data: apts }] = await Promise.all([
      fetchSettings(),
      supabase.from('apartments').select('slug, name, cancellation_days, deposit_percentage').order('name'),
    ]);
    ...
    (apts || []).forEach(apt => {
      ...
      const hasDepositOverride = apt.deposit_percentage != null && apt.deposit_percentage !== deposit;
  ...
  const handleSave = async () => {
    ...
    setSuccess(null);
    try {
      await Promise.all(apartments.map(apt => {
      ...
      setSuccess('✓ Excepciones guardadas correctamente');
  ...
  return (
    <>
      <div className="pl-6 pr-6 py-6 border-b border-gray-200 bg-white">
        ...
        <div className="text-sm text-gray-600 mt-1">Excepciones por apartamento respecto a la política global</div>
      </div>
      ...
      <div className="p-6 bg-gray-50 space-y-6 pb-24">
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-5 max-w-3xl flex items-start gap-4">
          ...
          <div className="flex-1">
            ...
            <div className="text-sm text-blue-800">
              Cancelación gratuita hasta <strong>{globalDays} días</strong> antes · Depósito <strong>{globalDeposit}%</strong> al reservar
            </div>
            <Link
        ...
        <div className="bg-white border border-gray-200 rounded-xl max-w-3xl overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-100">
            ...
            <div className="text-xs text-slate-500 mt-0.5">Deja en «Usando global» para seguir la política por defecto, o activa una excepción personalizada</div>
          </div>

          <div className="divide-y divide-gray-100">
            {apartments.map(apt => {
              const ov = overrides[apt.slug] || { days: globalDays, deposit: globalDeposit, useGlobal: true };
              return (
                <div key={apt.slug} className={`p-5 ${ov.useGlobal ? 'bg-white' : 'bg-amber-50/40'}`}>
                  <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <button
                        type="button"
                        onClick={() => ov.useGlobal
                          ? setOverrides(prev => ({ ...prev, [apt.slug]: { ...prev[apt.slug], useGlobal: false } }))
                          : resetToGlobal(apt.slug)
                        }
                        className={`text-xs transition-colors ${ov.useGlobal
                  ...
                  {!ov.useGlobal && (
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="block text-xs font-semibold text-slate-500 mb-1.5">Días para cancelar</label>
                        <div className="flex items-center gap-3">
                          <input
                            type="range" min={1} max={60} value={ov.days}
                      ...
                      <div>
                        <label className="block text-xs font-semibold text-slate-500 mb-1.5">Depósito</label>
                        <div className="flex items-center gap-3">
                          <input
                            type="range" min={10} max={100} step={5} value={ov.deposit}
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 max-w-3xl text-xs text-amber-900 leading-relaxed">
          ⚠️ Los cambios solo aplican a nuevas reservas. Las reservas existentes mantienen las condiciones con las que fueron creadas.
        </div>
      </div>
      ...
      <div className="fixed bottom-0 right-0 left-0 bg-white border-t border-gray-200 py-4 px-6 flex justify-between items-center z-50">
        <span className="text-xs text-gray-500">
          {apartments.filter(a => overrides[a.slug] && !overrides[a.slug].useGlobal).length} apartamento(s) con excepción activa
        </span>
        <button
      ...
      {success && (
        <div className="fixed bottom-20 right-6 bg-green-500 text-white px-5 py-3 rounded-lg text-xs font-semibold z-[1001] shadow-lg">
          {success}
        </div>
      )}
      {error && (
        <div className="fixed bottom-20 right-6 bg-red-500 text-white px-5 py-3 rounded-lg text-xs font-semibold z-[1001] shadow-lg max-w-xs">
          ✗ {error}
        </div>
      )}
