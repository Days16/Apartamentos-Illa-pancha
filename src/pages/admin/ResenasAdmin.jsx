...
export default function ResenasAdmin() {
  ...
  function startEdit(r) {
    setForm({ name: r.name || '', origin: r.origin || '', text: r.text || '', stars: r.stars || 5, date: r.date || '', apt: r.apt || '', active: r.active !== false });
    setEditingId(r.id);
    setShowForm(true);
    setMsg(null);
  ...
  async function handleSave(e) {
    ...
    if (!form.name.trim() || !form.text.trim()) {
      setMsg({ type: 'error', text: 'El nombre y el texto son obligatorios.' });
      return;
    }
    setSaving(true);
    setMsg(null);
    try {
      ...
      if (editingId) {
        ...
        setMsg({ type: 'ok', text: 'Reseña actualizada.' });
      } else {
        const { error } = await supabase.from('reviews').insert(payload);
        if (error) throw error;
        setMsg({ type: 'ok', text: 'Reseña creada.' });
      }
      await loadData();
      cancelForm();
    } catch (err) {
    ...
    <div className="p-6 max-w-4xl">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Gestiona las opiniones que aparecen en la web. Solo se muestran las activas.</p>
        </div>
        {!showForm && (
          <button onClick={startNew} className="bg-[#1a5f6e] text-white px-4 py-2 rounded-lg text-sm font-semibold hover:bg-[#154e5b] transition-colors">
            + Nueva reseña
          </button>
        )}
      </div>
      ...
      {/* FORMULARIO */}
      {showForm && (
        <form onSubmit={handleSave} className="bg-white rounded-xl border border-gray-200 p-6 mb-8 shadow-sm">
          <h2 className="font-bold text-slate-700 mb-5">{editingId ? 'Editar reseña' : 'Nueva reseña'}</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-600 mb-1">Fecha (texto)</label>
              <input
                type="text"
                value={form.date}
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-600 mb-1">Apartamento (opcional)</label>
              <select
                value={form.apt}
              ...
              >
                ...
                {apts.map(a => <option key={a.slug} value={a.slug}>{a.name}</option>)}
              </select>
            </div>
          </div>
          ...
          <div className="flex gap-3">
            <button type="submit" disabled={saving} className="bg-[#1a5f6e] text-white px-5 py-2 rounded-lg text-sm font-semibold hover:bg-[#154e5b] transition-colors disabled:opacity-50">
              {saving ? 'Guardando...' : editingId ? 'Actualizar' : 'Crear reseña'}
            </button>
            <button type="button" onClick={cancelForm} className="px-5 py-2 rounded-lg text-sm text-slate-600 border border-gray-300 hover:bg-gray-50 transition-colors">
              Cancelar
            </button>
          </div>
        </form>
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {reviews.map(r => (
            <div key={r.id} className={`bg-white rounded-xl border p-5 flex gap-4 items-start transition-opacity ${r.active ? 'border-gray-200' : 'border-gray-100 opacity-60'}`}>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-3 mb-1 flex-wrap">
                  ...
                  {!r.active && <span className="text-xs bg-gray-100 text-gray-500 px-2 py-0.5 rounded">Oculta</span>}
                </div>
